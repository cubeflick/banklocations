			...
			if(arguments.length == 4){
					grid.forEachRow(function(id){
						...
						if(!c.cell._brval&&!c.cell._cellType){
							if(c.cell.combo_value=="") c.setCValue(c._combo_pre+"&nbsp;");
							else{
								if(!combo.getOption(c.cell.combo_value))
									c.setCValue(c._combo_pre+c.cell.combo_value);
								else c.setCValue(c._combo_pre+combo.getOption(c.cell.combo_value).text);
							}
						}
					});	
				}
					...
					if(obj.getAttribute("text")) {
						if(obj.getAttribute("text")._dhx_trim()=="") c.setCValue(c._combo_pre+"&nbsp;"); 
						else c.setCValue(c._combo_pre+obj.getAttribute("text")); 
					}
					else{
						if((!c.cell.combo_value)||(c.cell.combo_value._dhx_trim()=="")) c.setCValue(c._combo_pre+"&nbsp;");
						else{
							if(!combo.getOption(c.cell.combo_value))
								c.setCValue(c._combo_pre+c.cell.combo_value);
							else c.setCValue(c._combo_pre+combo.getOption(c.cell.combo_value).text);
	...
	if(obj.getAttribute("source")){
		if(obj.getAttribute("auto")&&convertStringToBoolean(obj.getAttribute("auto"))){
			...
			if(obj.getAttribute("xmlcontent")){
				var c = (this.cell)?this:grid.cells(idd,index);
				if(obj.getAttribute("text")) c.setCValue(c._combo_pre+obj.getAttribute("text"));
			}
			else{
				grid.forEachRow(function(id){
					var c = grid.cells(id,index);
