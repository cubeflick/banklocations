...
function dhtmlXTabBar(parentObject,mode,height){mode=mode||"top"
 dhtmlxEventable(this);this._s={};this._c={};this._s.mode=mode;this._s.scrolls=true;this._s.line_height=(parseInt(height)||20)+3;this._s.skin_line = 1;this._s.tab_margin = 0;this._s.expand = 0;this._s._bMode=(mode=="right"||mode=="bottom")?1:0;this._s._vMode=(mode=="right"||mode=="left")?1:0;this._dx=this._s._vMode?"height":"width";this._dy=this._s._vMode?"width":"height";switch(mode){case "top":
 ...
 _getSkin:function(tab){if (tab && tab.skin)return this._styles[tab.skin];return this._a},
 ...
 };var f = function(){this._lineA.style[this._dx]="1px";var _quirks=(_isIE && document.compatMode == "BackCompat");var w = this._tabAll[this._s._vMode?"offsetHeight":"offsetWidth"]+(_quirks?2:0);if (this._lastActive)w=Math.max(w,this._lastActive.parentNode[this._s._vMode?"scrollHeight":"scrollWidth"]);if (w<6)return;d1.style[this._py]=parseInt(this._conZone.style[this._py])-3+"px";d1.style[this._dx]=w-2+"px";d2.style[this._py]=parseInt(this._conZone.style[this._py])-3+"px";d2.style[this._dx]=w-(_quirks?6:4)+(this._s.expand?2:0)+"px";this._lineA.style[this._dx]=w-2+"px"};f.call(this);var bf = this._checkScroll;this._checkScroll=function(){f.apply(this,arguments);bf.apply(this,arguments)}};if (this._a.data_color && this._conZone)this._conZone.style.backgroundColor=this._a.data_color},

 
 enableAutoReSize:function(){var self=this;dhtmlxEvent(window,"resize",function(){self._setSizes()})
 },
 ...
 _setRowSizes:function(){for (var i=0;i<this._rows.length;i++){this._rows[i].style[this._dy]=this._s.line_height+"px";this._rows[i].style[this._py]=i*(this._s.line_height-(this._s.skin_line_x?4:2))-((_isIE && !window.postMessage && this._s._bMode)?2:0)+"px";this._rows[i].style.zIndex=10+i};this._setSizes()},
 ...
 addTab:function(id, text, size, position, row){row=row||0;for (var i=this._rows.length;i<=row;i++)this._createRow();var z=this._rows[row].tabCount;if ((!position)&&(position!==0)) position=z;var tab=this._createTab(id, text, (size=="*"?10:size||this._s._tabSize));this._addTab(this._rows[row],tab,size,position);this._tabs[id]=tab;this.cells(id).hide();this._checkScroll()},
 ...
 this._setTabSizes(row);delete this._tabs[id];this._setRowSizes();if (this._lastActive==tab)this._lastActive=null},
 ...
 _img:function(tag,y,pos,a,b){tag.style.backgroundImage="url("+this.imgUrl+this.skin+"/"+tag.parentNode.skin+"_"+this._s.mode+".gif)";tag.style.backgroundPosition=this._get_img_pos(y);tag.style[this._py]="0px";if (pos){tag.style[this._dx]=a+"px";tag.style[pos]=(b||0)+"px"}},
 ...
 adjustTabSize:function(tab,size){var a=this._getSkin(a);size=size||(tab.scrollWidth+10);tab.style[this._dx]=size+"px";tab._size=size;this._img(tab.childNodes[3],5,this._px,size-(a.left+a.right),a.left)},
 ...
 cells:function(id){if (!this._tabs[id])return null;if (!this._content[id]){var d=document.createElement("DIV");d.style.cssText="width:100%;height:100%;visibility:hidden;overflow:hidden;position:absolute;top:0px;left:0px;";d.setAttribute("tab_id",id);d.skin = this.skin;this._conZone.appendChild(d);(new dhtmlXContainer(d)).setContent(d);this._content[id]=d;var self=this;d.show = function(){if (self._s.hide)this.style.display="";this.style.visibility="visible";this.style.zIndex="1";if (self._awdj || self._ahdj){var dim = this._getContentDim();if (self._awdj)self.entBox.style.width=dim[0]+2+"px";if (self._ahdj)self.entBox.style.height=dim[1]+self._rows.length*(self._s.line_height-2)+2+"px";self._setSizes();self._setTabTop(self._lastActive)};this.activate()};d.hide = function(){if (self._s.hide){this.style.visibility="visible";this.style.display="none"}else
 this.style.visibility="hidden";this.style.zIndex=-1;if (this._hrfmode=="iframe")this.attachURL("javascript:false")};d.activate=function(){if (this._delay)this._attachContent.apply(this,this._delay);this.adjustContent(this.parentNode,0,0,false,0);d.updateNestedObjects();if (this._hrfmode!="iframe")this._delay=null};d._doOnAttachStatusBar = d.activate;d._doOnAttachMenu = d.activate;d._doOnAttachToolbar = d.activate;d._getContentDim=function(){return [this.mainCont.scrollWidth,this.mainCont.scrollHeight]};d._doOnAttachURL=function(){self.callEvent("onXLE",[]);self.callEvent("onTabContentLoaded",[this.getAttribute("tab_id")])};d._doOnBeforeAttachURL=function(){self.callEvent("onXLS",[])}};return this._content[id]},

 
 forceLoad:function(id,href){if (href)this.setContentHref(id,href);this.cells(id).activate()},
 ...
 getLabel:function(tab){return this._tabs[id].firstChild.innerHTML}, 
 ...
 tabWindow:function(tab_id){return (this._content[tab_id]?this._content[tab_id]._frame:null)},
 ...
 setContentHref:function(id,href){switch (this._hrfmode){case "iframes":
